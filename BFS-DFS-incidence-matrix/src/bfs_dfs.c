...
Graph GraphInitialize(int V, int E) {
  ...
  G->incMatrix = (Vertex **)malloc(V * sizeof(Vertex));
  for(int i=0; i<V; i++)
    G->incMatrix[i] = (Vertex*)malloc(E * sizeof(Vertex));
  
  for(int i=0; i<V; i++) {
    for(int j=0; j<E; j++)
      G->incMatrix[i][j] = VertexInitialize(0);
  }
  return G;
}
